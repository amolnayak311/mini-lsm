...
impl MemTable {
    ...
    fn transform_bound(&self, bound: Bound<&[u8]>) -> Bound<Bytes> {
        match bound {
    ...
    pub fn scan(&self, lower: Bound<&[u8]>, upper: Bound<&[u8]>) -> MemTableIterator {
        // This is generated by oroboros #[self_referencing] and the iter_builder is
        // generated because we added these two
        // #[borrows(map)]
        // #[not_covariant]
        // in the struct
        let bounds = (self.transform_bound(lower), self.transform_bound(upper));
        ...
        .build();
        // Iterate once so that the item in the struct is populated
    ...
    pub fn flush(&self, builder: &mut SsTableBuilder) -> Result<()> {
        self.map.iter().for_each(|entry| {
            builder.add(
                KeySlice::from_slice(entry.key().clone().as_ref(), TS_DEFAULT),
                entry.value().as_ref(),
            );
        });
        Ok(())
...
impl StorageIterator for MemTableIterator {
    ...
    fn next(&mut self) -> Result<()> {
        let next_item: (Bytes, Bytes) = match self.with_iter_mut(|iter| iter.next()) {
            Some(e) => (e.key().clone(), e.value().clone()),
            None => (Bytes::new(), Bytes::new()),
        };
        self.with_mut(|x| *x.item = next_item);
        Ok(())
    }
}
